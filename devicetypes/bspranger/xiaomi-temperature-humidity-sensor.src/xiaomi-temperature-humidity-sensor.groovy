...
metadata {
    ...
    tiles(scale: 2) {
        ...
        valueTile("feelsLike", "device.feelsLike", decoration: "flat", inactiveLabel: false, width: 2, height: 2) {
            state "feelsLike", label:'${currentValue}°', icon: "st.Weather.weather2",
                backgroundColors:[
							// Celsius
							[value: 0, color: "#153591"],
							[value: 7, color: "#1e9cbb"],
        ...
        main(["temperature2"])
        details(["temperature", "battery", "feelsLike", "lastcheckin", "batteryRuntime"])
    }
}
...
def parse(String description) {
    ...
    def feelslike = (device.currentValue("temperature")+5/9*(6.112*10**(7.5*device.currentValue("temperature")/(237.7+device.currentValue("temperature")))*device.currentValue("humidity")/100-10))
    feelslike = (Math.round(feelslike * 10))/ 10 as Float
    sendEvent(name: "feelsLike", value: feelslike, displayed: false)

    log.debug "${device.displayName}: Parse returned ${map}"
    // log.info "Temp is ${device.currentValue("temperature")} and Humidity is ${device.currentValue("humidity")}"
